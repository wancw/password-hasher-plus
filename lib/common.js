 ...
 *
 * ***** END LICENSE BLOCK ***** */
const { PassHashCommon } = require('./passhashcommon');
const tld = require('./tld.min');
...
const default_hashkey = 'Ctrl+Shift+51';
const default_maskkey = 'Ctrl+Shift+56';
...
function dumpDatabase() {
  const entries = [];
  const keys = toArray(localStorage);
...
function grepUrl(url) {
  ...
  try {
    ...
    const address = split_url.exec(url)[3]; // a
