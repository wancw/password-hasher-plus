...
/*
 * Configurable const iables. You may need to tweak these to be compatible with
...
function str2binb(str) {
  ...
  for (let i = 0; i < str.length * chrsz; i += chrsz) {
    if (bin[i >> 5] != undefined) bin[i >> 5] |= (str.charCodeAt(i / chrsz) & mask) << (32 - chrsz - (i % 32));
    else bin[i >> 5] = (str.charCodeAt(i / chrsz) & mask) << (32 - chrsz - (i % 32));
  }
  /*for(const  i = 0; i < str.length * chrsz; i += chrsz)
...
function binb2b64(binarray) {
  ...
  for (let i = 0; i < binarray.length * 4; i += 3) {
    ...
    const b2 =
      binarray[(i + 1) >> 2] != undefined ? ((binarray[(i + 1) >> 2] >> (8 * (3 - ((i + 1) % 4)))) & 0xff) << 8 : 0;
